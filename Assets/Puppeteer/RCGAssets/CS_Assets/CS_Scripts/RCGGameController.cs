...
namespace RoadCrossing
{
	...
	public class RCGGameController : MonoBehaviour
	{
		...
		void Start()
		{
			AlertText.text = "USE THE KEYBOARD ARROWS, OR CLICK AROUND THE FROG TO MOVE";
			started = true;
			...
			if (connected) {
				...
				Debug.Log(address);
				if (address.Length > 6) {
					WalletAddress.text = address.Substring(0, 4) + "..." + address.Substring(address.Length - 4);
				}
			} else {
				AlertText.color = Color.red;
				AlertText.text = "You need to connect wallet";
			}
			//Update the score and lives without changing them
			ChangeScore(0);
			StartCoroutine(ChangeLives(0));
			StartIterativeFunction();
		...
		private void StartIterativeFunction()
    {
        
			StartCoroutine(IterativeFunctionCoroutine());
    }

		private IEnumerator IterativeFunctionCoroutine()
    {
    ...
    private IEnumerator GetCredit()
    {
        ...
        if (hs_get.error != null) {
            ...
            CreditAmount.text= json["credits"].ToString();
						if (json["credits"].ToString() == "0") {
							AlertText.color = Color.red;
							AlertText.text = "You don't have enough credit to play game";
						}
        }
    }
		...
		void Unpause()
		{
			// check if game is overed.
			if (started == true) {
				if (float.Parse(CreditAmount.text) > 0 ) {
					PayCredit();
				} else {
					AlertText.text = "You don't have enough credit to play";
				}
			} else {
				started = false;
				isPaused = false;
		...
		void PayCredit() 
    {
        if (address != "")
        {
            StartCoroutine (PayCreditIenum());
        }
    }

    private IEnumerator PayCreditIenum()
    {
        string post_url = apiUrl + "minusUserCredit?address=" + address;
				
        UnityWebRequest hs_post = UnityWebRequest.Post(post_url, address);
